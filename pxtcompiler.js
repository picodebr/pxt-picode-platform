...
(function (ts_2) {
    ...
    (function (pxtc) {
        ...
        (function (service) {
            ...
            service.getParameterTsType = getParameterTsType;
            ...
            function getCompletions(v) {
                ...
                if (isPython) {
                    ...
                    if (!isMemberCompletion && resultSymbols.length > MAX_SYMBOLS_BEFORE_FILTER) {
                        resultSymbols = resultSymbols
                            .filter(function (s) { return (isPython ? s.symbol.pyQName : s.symbol.qName).indexOf(partialWord) >= 0; });
                    }
                }
                else {
                    tsPos = position;
                ...
                if (resultSymbols.length === 0) {
                    ...
                    var wordMatching = allSymbols.filter(function (s) { return (isPython ? s.pyQName : s.qName).indexOf(partialWord) >= 0; });
                ...
                if (call) {
                    ...
                    if (paramIdx >= 0) {
                        ...
                        if (callSym) {
                            ...
                            var paramType = getParameterTsType(callSym, paramIdx, blocksInfo_1);
                            if (paramType) {
                ...
                resultSymbols
                    .map(function (sym) { return sym.symbol.attributes.alias ? completionSymbol(service.lastApiInfo.apis.byQName[sym.symbol.attributes.alias], sym.weight) : sym; })
        ...
        (function (service_1) {
            ...
            var operations = {
                ...
                apiSearch: function (v) {
                    ...
                    if (!service_1.lastFuse || search.subset) {
                        var weights_1 = {};
                        ...
                        service_1.lastFuse = new Fuse(searchSet, fuseOptions);
                    }
                    var fns = service_1.lastFuse.search(search.term);
                    return fns.slice(0, SEARCH_RESULT_COUNT);
                },
                projectSearch: function (v) {
                    var search = v.projectSearch;
                    var searchSet = search.headers;
                    if (!service_1.lastProjectFuse) {
                        var fuseOptions = {
                            shouldSort: true,
                            threshold: 0.6,
                            location: 0,
                        ...
                        service_1.lastProjectFuse = new Fuse(searchSet, fuseOptions);
                    }
                    var fns = service_1.lastProjectFuse.search(search.term);
                    return fns;
                },
                projectSearchClear: function () {
                    service_1.lastProjectFuse = undefined;
                }
            };
                ...
                function getParameterDefault(param) {
                    ...
                    if (shadowSymbol) {
                        ...
                        if (recursionDepth < 3 && service.lastApiInfo.decls[shadowSymbol.qName]) {
                            var snippet_2 = getSnippet(context, shadowSymbol, service.lastApiInfo.decls[shadowSymbol.qName], python, recursionDepth + 1);
                            if (snippet_2)
                                return snippet_2;
                        }
                    }
