    ...
    (function (editor) {
        ...
        var MonacoTilemapEditor = /** @class */ (function (_super) {
            ...
            MonacoTilemapEditor.prototype.textToValue = function (text) {
                ...
                var allTiles = pxt.react.getTilemapProject().getProjectTiles(tm.tileset.tileWidth);
            ...
            MonacoTilemapEditor.prototype.readTilemap = function (text) {
                ...
                var match = /^\s*tilemap\s*(?:`([^`]*)`)|(?:\(\s*"""([^"]*)"""\s*\))\s*$/.exec(text);
                var name = (match[1] || match[2] || "").trim();
                if (name) {
                    var id = ts.pxtc.escapeIdentifier(name);
                    var proj = project.getTilemap(id);
                    if (!proj) {
                        var _a = project.createNewTilemap(id, 16, 16, 16), name_1 = _a[0], map = _a[1];
                        proj = map;
                        id = name_1;
                    }
                    this.tilemapName = id;
